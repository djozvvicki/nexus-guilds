...
import pl.mrjozvvicki.myguilds.utils.ItemsChecker;
...
public class CommandManager {
    ...
    private static boolean handleListCommand(Player p, String[] args) {
        ...
        if (guildsManager.getGuildsCount() > 0) {
            Chat.sendMessage(p, "&7[&4MyGuilds&7] Lista gildii:");
            int index = 1;
            for (Guild guild : guilds) {
    ...
    private static boolean handleHelpCommand(Player p, String[] args) {
        Chat.sendMessage(p, "&7[&4MyGuilds&7] Dostępne komendy:");
    ...
    private static boolean handleInfoCommand(Player p, String[] args) {
        Chat.sendMessage(p, "&7[&4MyGuilds&7] Wersja: 0.1.0-SNAPSHOT");
        Chat.sendMessage(p, """
